...
const char* const KnUtils_RCS_ID = "@(#)class KnUtils definition.";
 ...
 * @param fill_width when [size]>0, the missing chars are filled with it.
 * @return the decimal representation of [val].
 */
string
KnUtils::sprintDec( uint val, uint size, char fill_with )
{
  char buf[ 128 ];
  snprintf( buf, 128, "%u", val );
  uint l = strlen( buf );
  string s( size >= l ? size - l : 0, fill_with );
  s += buf;
  return s;
}

/**
 * Builds a string displaying the value [val] as a hexadecimal representation.
 * @param val the value to display
 * @param size if >0, the value will take [size] chars.
 * @param fill_width when [size]>0, the missing chars are filled with it.
 ...
 */
string
KnUtils::sprintHex( uint val, uint size, char fill_with )
{
  char buf[ 128 ];
  snprintf( buf, 128, "%x", val );
  uint l = strlen( buf );
  string s( size >= l ? size - l : 0, fill_with );
  s += buf;
  return s;
 ...
 */
string
KnUtils::sprintBin( uint val, uint size, char fill_with )
{
  char buf[ 129 ];
  buf[ 128 ] = '\0';
  uint idx = 128;
  ...
  idx = 128 - size < last_one ? 128 - size : last_one;
  string str( buf + idx );
  //str.insert( idx, buf, 128 - idx );
  return str;
}
 ...
 */
void
KnUtils::binomials( uint n, uint* t )
{
  uint i, j;
  t[ 0 ] = 1;
  for ( j = 1; j <= n; ++j )
 ...
 */
void 
KnUtils::selfDisplay( ostream& that_stream ) const
{
  that_stream << endl;
}
